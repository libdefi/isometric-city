...
function generateOceans(grid: Tile[][], size: number, seed: number): WaterBody[] {
  const waterBodies: WaterBody[] = [];
  const oceanChance = 0.6; // 60% chance of having at least one ocean connection
  
  if (Math.random() > oceanChance) {
    return waterBodies; // No oceans this time
  }
  
  // Determine which edges have oceans (1-3 edges)
  const edges: Array<'north' | 'south' | 'east' | 'west'> = ['north', 'south', 'east', 'west'];
  const numOceans = 1 + Math.floor(Math.random() * 3); // 1-3 ocean connections
  ...
  for (const edge of selectedEdges) {
    ...
    const edgeWidth = 3 + Math.floor(Math.random() * 4); // 3-6 tiles wide
    
    if (edge === 'north') {
      // Top edge
      const startY = 0;
      const endY = edgeWidth;
    ...
    if (oceanTiles.length > 0) {
      const avgX = oceanTiles.reduce((sum, t) => sum + t.x, 0) / oceanTiles.length;
      const avgY = oceanTiles.reduce((sum, t) => sum + t.y, 0) / oceanTiles.length;
